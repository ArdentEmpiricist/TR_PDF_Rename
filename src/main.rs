...
//check is path is file or directory
    if path.is_file() && path.extension().and_then(OsStr::to_str) == Some("pdf")  {
        ...
        for entry in read_dir(&path).expect("error parsing 'entry in read_dir(&path)'") {
            ...
            if file_path.is_file() && file_path.extension().and_then(OsStr::to_str) == Some("pdf") && entry.file_name().to_str().unwrap().starts_with("20") == false {
                println!("Renamed {:?} to {:?}", entry.file_name(), path.file_name().unwrap());
                    rename(&file_path)?;
            } else if file_path.is_file() && file_path.extension().and_then(OsStr::to_str) == Some("pdf") && entry.file_name().to_str().unwrap().starts_with("20") == true {
...
pub fn rename(path: &Path) -> std::io::Result<()> {
    ...
    date_ordertype_name.push_str("_");
